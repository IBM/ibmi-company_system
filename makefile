...
all: .logs .evfevent library $(PREPATH)/APP.BNDDIR $(PREPATH)/DEPTS.PGM $(PREPATH)/EMPLOYEES.PGM $(PREPATH)/MYPGM.PGM $(PREPATH)/NEWEMP.PGM $(PREPATH)/POPDEPT.PGM $(PREPATH)/POPEMP.PGM
...
$(PREPATH)/EXAMPLE.MODULE: qrpglesrc/example.rpgle
	liblist -c $(BIN_LIB);\
	liblist -a $(LIBL);\
	system "CRTRPGMOD MODULE($(BIN_LIB)/EXAMPLE) SRCSTMF('qrpglesrc/example.rpgle') OPTION(*EVENTF) DBGVIEW(*SOURCE) TGTRLS(*CURRENT) TGTCCSID(*JOB)" > .logs/example.splf || \
	(system "CPYTOSTMF FROMMBR('$(PREPATH)/EVFEVENT.FILE/EXAMPLE.MBR') TOSTMF('.evfevent/example.evfevent') DBFCCSID(*FILE) STMFCCSID(1208) STMFOPT(*REPLACE)"; $(SHELL) -c 'exit 1')
...
$(PREPATH)/EXAMPLE.FILE: example.sql
	liblist -c $(BIN_LIB);\
	liblist -a $(LIBL);\
	system "RUNSQLSTM SRCSTMF('example.sql') COMMIT(*NONE)" > .logs/example.splf
